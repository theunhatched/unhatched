...
const getSSRInitProps = (ssr) => (PageComponent) => async (ctx) => {
  const { AppTree } = ctx

  // Initialize ApolloClient, add it to the ctx object so
  // we can use it in `PageComponent.getInitialProp`.
  const headers = await getHeaders(ctx)
  ctx.apolloClient = initApolloClient(null, headers)
...
export const withApollo = ({ ssr = true } = {}) => (PageComponent) => {
  const WithApollo = ({ apolloClient, apolloState, ...pageProps }) => {
    // client = (on: getDataFromTree & next.js ssr) || (on: next.js csr)
    return (
      <ApolloProvider
        client={apolloClient || initApolloClient(apolloState, {})}
      >
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithApollo.getInitialProps = getSSRInitProps(ssr)(PageComponent)
  }
